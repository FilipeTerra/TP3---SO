...
// Simple LCG random number generator for the lottery scheduler.
...
// Per-CPU process scheduler (lottery).
// Each CPU calls scheduler() after setting itself up.
// Scheduler never returns.  It loops, doing:
//  - count total tickets among RUNNABLE processes
//  - draw a winning ticket at random
//  - run the winning process for one time slice
void
scheduler(void)
{
  struct proc *p;
  ...
  c->proc = 0;
  for(;;){
    // Enable interrupts briefly to avoid deadlock when all processes wait.
    intr_on();
    intr_off();
